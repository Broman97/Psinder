...
import com.example.psinder.ui.setup.SetupActivity;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        UsersRef.child(currentUserID).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                // Tu dokonalem zmian, mozesz zobaczyc sobie tez to na bazie,
                // robie przy setup activity zmieniam sobie miejsce w ktore wrzucam dane,
                // wiec Twoj link w miejscu ktorym znajduje sie zdjecie jest troche wyzej
                // niz te dane, jest to troche slabe, powinno byc w jednym miejscu, wiec
                // miejsce w ktorym je zapisuje, czyli to SetupActivity, powinno byc lekko zmodyfikowane
                // jednakze to co jest pod spodem dziala prawidlowo, w taki sposob tez powinienes odczytywac informacje
                // w innych miejscach w calej apce.
                for (DataSnapshot data : dataSnapshot.getChildren()) {
                    if (data.hasChildren()) {
                        UserSetupModel userSetupModel = data.getValue(UserSetupModel.class);

                        if (dataSnapshot.exists() && userSetupModel != null) {
                            if (userSetupModel.getFullname() != null && !userSetupModel.getFullname().isEmpty()) {
                                NavProfileUserName.setText(userSetupModel.getFullname());
                            }
